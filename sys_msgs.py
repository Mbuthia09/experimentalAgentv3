assistant_msg = {
    ...
    'content': """
        ...
        engine results that will be attached before a USER PROMPT. You must analyse the SEARCH RESULT
...
search_or_not_msg = """\
        ...
        with an AI assistant requires more data to be retrieved from searching Google to enable the assistant
        ...
        simply respond "True". If the conversation already has context, or a Google search is not what an
        ...
        using only the logic in these instructions.
    """


query_msg = """\
        You are not an AI assistant that responds to a user. You are a web search query generator model.
        You will be given a prompt to an AI assistant with web search caabilities. If you are being used, an AI
        has determined this prompt to the actual AI assistant, requires web search for more recent data.
        You must determine what data the assistant needs from search and generate the best possible DuckDuckGo query
        to find that data. Do not respond with anything but a query that an expert human search engine user would type
        into DuckDuckGo to find the needed data. Keep your queries simple,
...
best_search_msg = """\
        ...
        search result out of a list of ten results. The best search result is the link an expert human human
        search engine user would type into DuckDuckGo to find the data to respond to a USER_PROMPT after
        searching DuckDuckGo for the SEARCH_QUERY. \nAll user messages you receive in this conversation will have
        the format of: \n
            SEARCH_RESULTS: [{}, {}, {}] \n
        ...
        to this conversation should always be 1 token, being an integer between o-9
    """

contains_data_msg = """\
        You are not an AI agent that responds to a user. You are an AI model designed to analyse data scraped
